...
/*	 $("#flyTime").keydown(function(event){
    	...
    	$.ajax({
            ...
            success : function(datas) {
                if(datas){
                    ...
                    //时间保存起来，用于其它功能
                    var object = parent.supData;
                    object.startTime = formatDatee(day);
                    object.endTime = formatDatee(day);
                    //默认第三个图的往返
    	...
    	$(".sales-check .leglist li").bind("click",function(){
    		if($(this).attr("isN")==="true"){
				if($(this).hasClass("ck")){
					...
					$(this).addClass("ck");
					naviChip+=1;
					if($(this).text().split("")[0]== $("#dpt_AirPt_Cd").val() && $(this).text().split("")[1]==$("#arrv_Airpt_Cd").val()){
    ...
    $(window).resize(function(){	//重新计算窗口尺寸
        ...
        initalData(data);
        mid(saveData);
    	let lla = $("#goAndBackegs_Lod_FtsData").text();
    	let llb = $("#goegs_Lod_FtsData").text();
    	let llc = $("#backegs_Lod_FtsData").text();
        setTimeout(function(){
...
/*        var dpt = nameList[0];
        var arrv = nameList[nameList.length-1];
        var pas ='';*/
    ...
    function coll(data,tempdata,type,allFli){	//1、2表格
...
//        });
    	if(totaldata_income.length==0){	//第一次进入
        	...
        	if(xdata_date.length>=1){	//如果有日期
        		let a  = xdata_date[xdata_date.length-1];	//最后一天的数据
        		
            	if(allFli==9){
            		flightTime= parseFloat(objMap[a].goAndBack.stzsr / objMap[a].goAndBack.xssr).toFixed(2);
            		console.log(flightTime)
            		//$("#flyTime").val(parseFloat(objMap[a].goAndBack.stzsr / objMap[a].goAndBack.xssr).toFixed(2));
            	}
            	...
            	goAndBackset_Ktr_IneData = parseFloat(Agoset_Ktr_IneData / naviChip).toFixed(2);
        ...
        if((gostzsrData-backstzsrData)>0){
            ...
            //B小时收入
            if(naviChip==$("ul.leglist li").length){//全选
            	var L_all_current=data.success.newmap.everyList[0].dataMap.date;
            	let key={};
            	for(k in L_all_current){
            		key=L_all_current[k];
            	}
                $("#goxssrData").html(key.go.xssr);
                $("#backxssrData").html(key.back.xssr);
                $("#goAndBackxssrData").html(key.goAndBack.xssr);            	
            }else{
                $("#goxssrData").html("--");
                $("#backxssrData").html("--");
        ...
        if(naviChip==$("ul.leglist li").length){//全选
            ...
            $("#backxssrData").html(key.back.xssr);
            $("#goAndBackxssrData").html(key.goAndBack.xssr);
            if(key.goAndBack.xssr=="--"){
                $("#flyTime").val(key.goAndBack.xssr);            	
            }
            else{
            	setTimeout(function(){
            		$("#flyTime").val(flightTime)
            	} ,200);
            }
    	...
    	//theCurve("canvas",xdata_date,null,null,backdata_canvas,goegs_Lod_FtsData,backegs_Lod_FtsData,goAndBackegs_Lod_FtsData);        	
