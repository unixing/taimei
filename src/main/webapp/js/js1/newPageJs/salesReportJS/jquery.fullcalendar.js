		...
		if(typeof(datee) != 'undefined'&& datee != null && datee != '' && datee.indexOf('-')>=0){
			...
			if(datee!=""){
				...
				for(var i =0;i<$("td").length;i++){
					if($("td").eq(i).attr("abbr")==datee){
						$("td").eq(i).addClass("calendar-selected");
					}
				}
				for(var i =0;i<$("td").length;i++){
					if($("td").eq(i).attr("abbr")==dateae){
						$("td").eq(i).removeAttr("style");
						$("td").eq(i).removeClass("calendar-selected");
					}
				}
			}
		}
	                	 ...
	                	 for(var i=0;i<datas.success.dateList.length;i++){
	                         for(var j=0;j< $(".time-box").find("td").length;j++){
	                             if($("td").eq(j).attr("abbr")==datas.success.dateList[i]){
	                            	...
	                            	if($('td').eq(j).attr('abbr')==day){
	                            		$('td').eq(j).addClass('calendar-selected');
	                            	}
	                             }
	                         }
	                     }
