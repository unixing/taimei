 ...
 * @returns {Boolean}
 */
function upload(domId, prefix, backId) {
	var file_name = $('#' + domId).val();
	if (file_name == null || file_name.length < 1) {
		top.uppShow('必须选择一个文件才能上传');
		return;
	}
	$.ajaxFileUpload({
		url : contextPath + 'common/fileUpload/fileUpload?prefix=' + prefix
				+ '&resourceId=' + $('#resourceId').val() + '&rootDir='
				+ $('#rootDir').val() + '&initDate=' + $('#initDate').val(),
		secureuri : false,
		fileElementId : domId,
		data : [ $('#' + domId).attr('name') ],
		dataType : 'json',
		success : function(data, status) {
			top.data = data;
			top.uppShow(data.msgDesc);
			if (data.opResult == "0") {
				$('#' + backId).val(data.addr);
			}
		},
